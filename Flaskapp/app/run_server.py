...
class RunServerClassifier(object):
    ...
    def check_format(self, sample):
        ...
        try:
            # check if correct number and type of ec classes are being used
            ec_class_1 = set([int(i.split(':')[1].split('.')[0])
        ...
        try:
            ...
            ec_nums = set([len(i.split(':')[1].split('.'))
                          for i in sample_list])
    ...
    def run_classifier(self, sample):
        ...
        classes = [i[0]
                   for i in list(zip(self.categories, Y_pred[0])) if i[1] == 1]
    ...
    def similarity_dict_metacyc(self):
        ...
        EC_dict = {tup[0]: tup[1] for tup in list(
            zip(data_Metacyc_part['Map'], data_Metacyc_part['EC']))}
        EC_name = {tup[0]: tup[1] for tup in list(
            zip(data_Metacyc_part['Map'], data_Metacyc_part['Name']))}
    ...
    def get_ranking_single(self, valid, metacyc=True, kegg=True):
        ...
        if metacyc:
            ...
            for k, l in enumerate(data):
                ...
                if sim > 0:
                    lst = [['UNK', list(data_dict.keys())[
                        k], pathway_name, sim, perc]]
                    df_simM = df_simM.append(pd.DataFrame(lst, columns=[
                                             'Pathway_test', 'Pathway_train', 'Pathway_name', 'Similarity', 'Percentage']))
        ...
        if kegg:
            ...
            for k, l in enumerate(data):
                ...
                if sim > 0:
                    lst = [['UNK', list(data_dict.keys())[
                        k], pathway_name, sim, perc]]
                    df_simK = df_simK.append(pd.DataFrame(lst, columns=[
                                             'Pathway_test', 'Pathway_train', 'Pathway_name', 'Similarity', 'Percentage']))
