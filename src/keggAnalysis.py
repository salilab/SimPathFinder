...
class ExtractKEGGData(object):
    ...
    def createAllPartialAnnot(self):
        for annotLevel in range(1,4):
            ...
            self.df[temp] = self.df.EC.apply(self.applyPartialAnnot,annotLevel=annotLevel)
        return self.df
    
    def getPreds(self,tag='biosynthesis'):
        self.Y_pred=self.final_model.predict(self.X)[:,self.categories[0][tag]]
    ...
    def multiConfusionMatrix(self,tagY='biosynthesis',
        ...
        if target1=='Name' and target2=='Pos':
            ...
            print ('Wrong selections, target1 should be Name or Label Name and target2 should be Pos or Neg')
...
class ExtractKEGGDataControl(ExtractKEGGData):
    def __init__(self,*args, **kwargs): 
        super(ExtractKEGGDataControl, self).__init__(final_model='LRCmodel.pkl')
