...
class Metrics(object):
    def __init__(self, data_dir='labeldata/', 
        ...
        self.final_model=pickle.load(open(self.model_dir+ final_model, "rb"))
    ...
    def rocCurvePlot(self, categories=[], Yvalid_array=[], prediction_prob=[]):
        ...
        ax.grid(True)
        plt.legend(loc='lower right',fontsize=14,edgecolor='white', facecolor=(1, 1, 1, 0.1))
    ...
    def prCurvePlot(self, categories, Yvalid_array=[], prediction_prob=[]):
        ...
        ax.set_facecolor('white')
        ax.grid(True)
        plt.legend(loc='lower left',fontsize=14,edgecolor='white', facecolor=(1, 1, 1, 0.1))
        ax.tick_params(axis='both', which='major', labelsize=16)
        ax.set_xticks(ax.get_xticks()[1:])
        plt.tight_layout()    
        plt.show()
    ...
    def evaluateMultilabelMetrics(self, Y_test,Y_pred):
        ...
        for i,j in enumerate(Y_test.to_numpy()):
            ...
            if ((len(union_2)+len(inter)))>0:
                tempp=len(inter)/(len(union_2)+len(inter))
                prec.append(len(inter)/(len(union_2)+len(inter)))
            #if (len(union_1)+len(union_2))>0:
               # f1.append((2*tempp*tempr)/(tempp+tempr))
...
class combinedEvaluations(Metrics,ModelVectors):
    def __init__(self,final_model_name='RMmodel100.pkl',
        ...
        self.original_model=pickle.load(open(self.model_dir+ self.final_model_name, "rb"))
        ...
        if self.control:
            self.control_model=pickle.load(open(self.model_dir+ 'LRCmodel.pkl', "rb"))
...
class MetricsDNN(object):
    def __init__(self, data_dir='../labeldata/', 
        ...
        self.data_dir=data_dir
        self.model_dir=model_dir
        self.final_model=keras.models.load_model('../dnnmodels//my_model_LSTMFTune.h5')
    ...
    def rocCurvePlot(self, categories=[], Yvalid_array=[], prediction_prob=[]):
       ...
       # plt.plot(fpr["micro"], tpr["micro"],'--',lw=3,color=colors[i],
        ...
        ax.set_facecolor('white')
        ax.grid(True)
        plt.legend(loc='lower right',fontsize=16,edgecolor='white', facecolor=(1, 1, 1, 0.1))
        ax.set_xticks(ax.get_xticks()[1:])
        plt.tight_layout()    
        ax.tick_params(axis='both', which='major', labelsize=16)
...
class EvaluateMetricsDNN(MetricsDNN,ModelVectors):
    ...
    def cleanTextChar(self, num_words=10000,
        ...
        self.token=Tokenizer(num_words=num_words,oov_token=oov_token,char_level=True)
        ...
        self.paddedTrain=pad_sequences(seq_train,maxlen=len(max(train, test, valid)),padding=padding)
        seq_valid=self.token.texts_to_sequences(valid)
        self.paddedValid=pad_sequences(seq_valid,maxlen=len(max(train, test, valid)),padding=padding)
        seq_test=self.token.texts_to_sequences(test)
        self.paddedTest=pad_sequences(seq_test,maxlen=len(max(train, test, valid)),padding=padding)
...
class EvaluateMetricsAnnot3DNN(EvaluateMetricsDNN):
    ...
    def cleanTextChar(self, num_words=10000,
        ...
        self.paddedValida3=pad_sequences(seq_valida3,maxlen=len(max(train, test, valid)),padding=padding)
        seq_valida2=self.token.texts_to_sequences(valida2)
        self.paddedValida2=pad_sequences(seq_valida2,maxlen=len(max(train, test, valid)),padding=padding)
        seq_valida1=self.token.texts_to_sequences(valida1)
        self.paddedValida1=pad_sequences(seq_valida1,maxlen=len(max(train, test, valid)),padding=padding)
