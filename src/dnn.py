...
class DNNModel(ModelVectors):
    ...
    def createEmbMatrix(self,):
        self.embedding_matrix_ft = np.random.random((len(self.token.word_index) + 1, self.model.vector_size))
        pas = 0
    ...
    def getClassWeights(self,):
        ...
        for i in range(number_dim):
            self.weights[i] = compute_class_weight('balanced', [0.,1.], self.Y_train[:, i])
        self.class_dict={i:0.1*self.weights[i,1] for i in range(len(self.categories))}
